...
class Layer:
    ...
    class P: # model parameters
        ...
        def set_params(self, neuron_params):
            ...
            if 'syn_delay' in neuron_params.keys():
                self.syn_delay = neuron_params['syn_delay']
...
#         print(target, weight, delay, target-self.id_0-1)
    ...
    def phi(self):
        ...
        # set negative values to 0.0 so that power on fraction number can be calculated
        idx_neg = np.where(V_diff < 0) 
        V_diff[idx_neg] = 0.0
        # it is not necessary to clip phi to 1.0, as firing rate >= 1.0 will act the same as 1.0
    ...
    def evaluate(self):
        ...
        self.S_.V_m = self.S_.V_m * self.V_.rho + self.S_.I_syn_ex * self.V_.zeta_ex + \
        ...
        self.S_.I_syn_in = self.S_.I_syn_in * self.V_.xi_in + self.B_.weighted_spikes_in[0]
...
#         self.S_.is_ref[idx_spiked] = self.V_.ref_count
        ...
        self.net.send_spike(idx_spiked + self.id_0 + 1, self.P_.syn_delay)   
...
#     connection = {}
    ...
    class Connection: # class storing connection dictionary
        ...
        def add_connection(self, source, target, weight): #, delay):
            if type(source) == int:
                if type(target) == int:
                    assert type(weight) == float #type(delay) == float
                else:
...
#                                                    'delay':np.array(delay[idx])}
    ...
    def send_spike(self, id_, delay):
        for idx, source_id in enumerate(id_):
            if self.C_:
                # Need to go through coonection by connection as there may exist multapses;
