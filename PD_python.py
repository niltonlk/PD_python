...
#         print(target, weight, delay, target-self.id_0-1)
    ...
    def evaluate(self):
        ...
        # evolve synaptic currents without considering inputs from other neurons
...
#         U_i = np.random.uniform(size=self.N)
        ...
        # Send spike event
        # the neuron ID in the network is the index + id_0
        self.net.send_spike(idx_spiked + self.id_0)   
...
class Network:
    ...
    class Buffer:
        ...
        def add(self, post_list):
            self.buffer = np.r_[self.buffer, post_list]

        def read(self, id_0, id_N, t_run):
            self.idx_del = np.where((self.buffer[:,2]<=t_run) & (self.buffer[:,2]>0.0))[0] # find the index of spikes arriving at time t_run
            self.spk = self.buffer[self.idx_del,:] # list of spikes arriving at time t_run
            idx_spk = np.where((self.spk[:,0]>=id_0) & (self.spk[:,0]<id_N))[0] # find index of neuron in the current layer
            return self.spk[idx_spk,:]

        def delete(self):
            if self.idx_del.size:
                self.buffer = np.delete(self.buffer, obj=self.idx_del, axis=0)
    ...
    def create_layer(self, N=[]):
        if type(N) == int:
            assert N > 0
            if len(self.N):
                self.layer.append(Layer(net=self, N=N, id_0=int(np.cumsum(self.N)[-1])+1,
                                        seed_phi=self.seed_phi+len(self.N),
                                        seed_poisson=self.seed_poisson+len(self.N)))
            else:
                self.layer.append(Layer(net=self, N=N, id_0=1,
                                        seed_phi=self.seed_phi+len(self.N),
                                        seed_poisson=self.seed_poisson+len(self.N)))
            ...
            for n_ in N:
                assert n_ > 0
                if len(self.N):
                    self.layer.append(Layer(net=self, N=n_, id_0=int(np.cumsum(self.N)[-1])+1,
                                            seed_phi=self.seed_phi+len(self.N),
                                            seed_poisson=self.seed_poisson+len(self.N)))
                else:
                    self.layer.append(Layer(net=self, N=n_, id_0=1,
                                            seed_phi=self.seed_phi+len(self.N),
                                            seed_poisson=self.seed_poisson+len(self.N)))
...
#    def create_connection(self, conn_params={}):
    ...
    def send_spike(self, id_):
        for idx, source_id in enumerate(id_):
            if self.C_: # case there is connection; the code bellow is necessary. only the comparison is added for test purpose, in case when simulating independent neurons
                self.spk_buffer.add(np.c_[self.C_.conn[source_id]['target'],
                                          self.C_.conn[source_id]['weight'],
                                          self.C_.conn[source_id]['delay']+self.t])
...
#             print(source_id, self.t)
    ...
    def simulate(self, time):
        ...
        while (self.t <= time):
            ...
            #print('{}/{}              '.format(self.t,time),end='\r')
